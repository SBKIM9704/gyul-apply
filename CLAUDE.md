...
동시에 개발로도 방송에 조용히 보탬이 될 수 있음("기타 부문")을 담백하게 덧붙이는 것이 목적. (과장·자기어필 톤은 지양)

## 지원자 정보

- 카페 아이디 / 숲(SOOP) 아이디: `mar5924`
- 활동명: 좀놀던괴물
- 채팅매니저 경력: 없음 (시청 데이터로 대체 — 지원서 문구엔 "개발 역량" 같은 자기어필 표현 미노출)
- 활동 가능 시간: 평일 20시 이후 · 주말 풀타임
  - 평일 07:00~20:00은 업무 및 출퇴근 시간이라 활발한 활동 어려움 (지원서에 명시할 것)
...
1. **지원 정보** — 상단 **아이디 복사 카드**(`.id-copy`: 카페·숲 통합, mar5924 클릭 복사) + 채팅매니저 경력(전체 폭 `.info-card.wide`). 과장을 피하려 "본업" 카드는 제거
...
3. **기타 부문** — 카드 **1개로 통합**. 핵심 메시지는 "**간단·반복 업무를 개발로 덜어 드리고 싶다**"(겸손 톤). 노래책 사이트는 "예전에 만들었다 나서는 것 같아 묵혀 둔" **참고용** 링크로 격하(`.btn` "노래책 사이트 (참고용) ↗", raw URL 미노출) (과거엔 노래책·AI 개발·서포트 툴 3개 카드 → 산만해 1개로 통합)
...
- 평일: 00~02 조건부(cond, 업무 전날 · 새벽 2시까지), 07~20 업무 및 출퇴근(work), 20~24 가능(avail), 그 외 휴식
...
- 수치 변경 → `months` 배열 + `chart-foot`의 하드코딩 요약 문구(누적·점유율·1위 달) 함께 갱신
